...
# Documentação customizada com segurança OAuth2
def custom_openapi():
...
# Simula credenciais de médicos (login)
...
def criar_token_acesso(dados: dict, expires_delta: timedelta = None):
    to_encode = dados.copy()
    expira = datetime.utcnow() + (expires_delta or timedelta(minutes=15))
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    medico = autenticar_medico(form_data.username, form_data.password)
    if not medico:
